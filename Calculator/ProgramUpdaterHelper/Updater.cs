...
namespace ProgramUpdater
{
    ...
    public class Updater
    {
        ...
        private readonly string appVersionsJsonUrl;

        private AppVersionsJson.Entry? appEntry;

        public Updater(string baseDir, Version clientVersion, Guid appGuid, string exeLaunchPath, string appVersionsJsonUrl)
        {
            ...
            this.appVersionsJsonUrl = appVersionsJsonUrl;
        }
        ...
        {
            using var response = await Client.GetAsync(appVersionsJsonUrl);
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var jsonObj = JsonConvert.DeserializeObject<AppVersionsJson>(data);

                // find specific record, based on GUID
                var entry = jsonObj?.AppEntries?.FirstOrDefault(x => x.GUID.Equals(AppGUID));
